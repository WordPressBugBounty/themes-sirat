jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.sirat-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message sirat-plugin-install');
                button.addClass('button-primary sirat-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.sirat-recommended-plugins .sirat-activation-note').css('display','block');
    ...
    $(document).on('click', '.sirat-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .sirat-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .sirat-recommended-plugins').css('display','none');
    });
